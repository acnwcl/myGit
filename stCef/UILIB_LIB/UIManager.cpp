...
m_pmImageHash(&m_mDefaultImageHash)
{
	m_skn.Empty();
...
BOOL CPaintManagerUI::IsExist(STRINGorID skn)
{
	USES_CONVERSION;
   	if(skn.m_lpstr != NULL)
	{
		LPCOLESTR lpwFileName = T2COLE( skn.m_lpstr );	// 转换T类型为宽字符
		HRESULT hr = ::StgIsStorageFile( lpwFileName );	// 是复合文件吗？
		if(FAILED(hr))
			return FALSE;
	}
	return TRUE;
...
void CPaintManagerUI::ChangePics(CStdStringPtrMap* pHashImage)
{
	m_pmImageHash = pHashImage;
}

BOOL CPaintManagerUI::ChangeUI(STRINGorID skn, STRINGorID xml, STRINGorID type, IDialogBuilderCallback* pCallback)
{
	if(!IsExist(skn))  //文件不存在
		return FALSE;

	if(m_oldskn.IsEmpty())    //新旧对比  相同则直接FALSE
	{
	   TCHAR path[MAX_PATH];
       _tcscpy(path,this->GetSkn());
	  m_oldskn=path;
	}
	else
	{
		if(m_oldskn == skn.m_lpstr) 
			return FALSE;	
	}
	RemoveSkn();
	RemoveAllImages();
	CDialogBuilder builder;
	if(m_pRoot!=builder.ChangeUI(skn, xml, (UINT)0,  pCallback, this))
	{
		builder.ChangeUI(m_oldskn.GetData(),xml,(UINT)0,pCallback,this);
	  		return FALSE;
	}	
	if(!m_oldskn.IsEmpty())
		m_oldskn=skn.m_lpstr;
	RECT rc;
	::GetWindowRect(m_hWndPaint,&rc);
	::InvalidateRect(m_hWndPaint,&rc,TRUE);
	Invalidate(rc);
	return TRUE;
}
           ...
           if( m_bUpdateNeeded ) {
               ...
               if( !::IsRectEmpty(&rcClient) ) {
                   ...
                   if( m_bFirstLayout ) {
                       ...
                       SendNotify(m_pRoot, MST_WINDOWINIT);
                   }
               }
           }
...
{	// Pretranslate Message takes care of system-wide messages, such as
    ...
    for( int i = 0; i < m_aPreMessages.GetSize(); i++ ) {
		...
		{
			for(int j=0;j<pT->m_aActivexs.GetSize();j++)
			{
				CActiveXUI* pCtrl = static_cast<CActiveXUI *>(pT->m_aActivexs[j]);
				CComQIPtr<IWebBrowser2> ie;
				pCtrl->GetControl(IID_IWebBrowser2, (void**)&ie);
				if(ie == NULL) continue;

				IServiceProvider* pServiceProvider = NULL;
				if (SUCCEEDED(ie->QueryInterface(
					...
					(void**)&pServiceProvider)))
				{
						...
						(void**)&pWindow)))
					{
						...
						if (SUCCEEDED(pWindow->GetWindow(&hwndBrowser)))
						{
							...
							pCtrl->GetControl(IID_IOleInPlaceActiveObject,(LPVOID*) &pInPlaceActiveObject);
							if(hwndBrowser == pMsg->hwnd && pInPlaceActiveObject && pInPlaceActiveObject->TranslateAccelerator(pMsg)==S_OK)
								return TRUE;
...
void CPaintManagerUI::SetFocus(CControlUI* pControl)
{
    ...
    if( m_pFocus != NULL ) 
    {
        ...
        event.dwTimestamp = ::GetTickCount();
        m_pFocus->Event(event);
        SendNotify(m_pFocus, MST_KILLFOCUS);
        m_pFocus = NULL;
    }
        ...
        && pControl->IsEnabled() ) 
    {
        ...
        event.dwTimestamp = ::GetTickCount();
        m_pFocus->Event(event);
        SendNotify(m_pFocus, MST_SETFOCUS);
    }
}
...
void CPaintManagerUI::SendNotify(CControlUI* pControl, DWORD pstrMessage, WPARAM wParam /*= 0*/, LPARAM lParam /*= 0*/)
{
    TNotifyUI Msg;
...
{
    TImageInfo* data = static_cast<TImageInfo*>(m_pmImageHash->Find(bitmap));
...
TImageInfo* CPaintManagerUI::GetImageEx(LPCTSTR bitmap, LPCTSTR type, DWORD mask)
{
    ...
    if( !data ) {
        if( AddImage(bitmap, type, mask) ) {
            data = static_cast<TImageInfo*>(m_pmImageHash->Find(bitmap));
        }
    }
...
TImageInfo* CPaintManagerUI::AddImage(LPCTSTR bitmap, LPCTSTR type, DWORD mask)
{
    ...
    if( type != NULL ) {
        if( isdigit(*bitmap) ) {
			...
			data = CRenderEngine::LoadImage(iIndex, m_skn.GetData(), type, mask);
        }
    }
    else {
		data = CRenderEngine::LoadImage(bitmap, m_skn.GetData(), type, mask);
    }

    if( !data ) return NULL;
    if( !m_pmImageHash->Insert(bitmap, data) ) {
...
bool CPaintManagerUI::RemoveImage(LPCTSTR bitmap)
{
    ...
    return m_pmImageHash->Remove(bitmap);
}

void CPaintManagerUI::RemoveAllImages()
{
    TImageInfo* data;
    for( int i = 0; i< m_mDefaultImageHash.GetSize(); i++ ) {
        if(LPCTSTR key = m_mDefaultImageHash.GetAt(i)) {
            data = static_cast<TImageInfo*>(m_mDefaultImageHash.Find(key));
            ::DeleteObject(data->hBitmap);
            delete data;
        }
    }

    m_mDefaultImageHash.Resize();
}
...
void CPaintManagerUI::RemoveAllDefaultAttributeList()
{
    ...
    for( int i = 0; i< m_pmImageHash->GetSize(); i++ ) {
        if(LPCTSTR key = m_pmImageHash->GetAt(i)) {
            pDefaultAttr = static_cast<CStdString*>(m_pmImageHash->Find(key));
            delete pDefaultAttr;
        }
    }
...
LPCTSTR CControlUI::GetFtImage()
{
	return m_sFtImage;
}

void CControlUI::SetFtImage(LPCTSTR pStrImage)
{
	if( m_sFtImage == pStrImage ) return ;

	m_sFtImage = pStrImage;
	Invalidate();
}
...
void CControlUI::SetPos(RECT rc)
{
    ...
    if( m_bFloat ) {
        if( !m_bFloatSetPos ) {
            m_bFloatSetPos = true;
            m_pManager->SendNotify(this, MST_SETPOS);
            m_bFloatSetPos = false;
        }
...
bool CControlUI::IsTrueVisible() const
{
	return m_bVisible;
}
...
void CControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("ftimage")) == 0 ) SetFtImage(pstrValue);
...
void CControlUI::PaintFtImage(HDC hDC)
{
	if( m_sFtImage.IsEmpty() ) return;
	if( !DrawImage(hDC, (LPCTSTR)m_sFtImage) ) m_sFtImage.Empty();
}
...
void CPaintManagerUI::SetSkn(LPCTSTR skn)
{	
	m_skn = skn;
	return ;
}

LPCTSTR CPaintManagerUI::GetSkn()
{
	return m_skn;
}

void CPaintManagerUI::RemoveSkn(void)
{
	m_skn.Empty() ;
	return ;
}
