            ...
            case _T('p'):  // Paragraph
                {
                    pstrText++;
//                  pt.x = rc.right;
					int nSpacing = (int) _tcstol(pstrText, const_cast<LPTSTR*>(&pstrText), 10);
                    cyLine = MAX(cyLine, tm.tmHeight + tm.tmExternalLeading) + nSpacing;
        ...
        else
        {
            ...
            if( cchChars > 0 ) {				
				...
				if( bDraw ) {
					...
					{
						RECT rect = {ptPos.x,ptPos.y,rc.right,rc.bottom};
						::DrawText(hDC,pstrText,_tcslen(pstrText),&rect,uStyle);
					}
