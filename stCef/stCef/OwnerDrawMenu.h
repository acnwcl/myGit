...
public:
	...
	ImageInfo* m_PngImageList;
	COLORREF m_ColorHotText;

protected:

	ImageInfo* GetImage(DWORD ResId)
	{
		DWORD dwSize = 0;
		HMODULE hInstance = ::GetModuleHandle(NULL);
		HRSRC hResource = ::FindResource(hInstance, MAKEINTRESOURCE(ResId), _T("png"));
	...
	void MeasureItem(LPMEASUREITEMSTRUCT lpMeasureItemStruct)
	{
		...
		else
		{
			...
			lpMeasureItemStruct->itemHeight = cy + 5;
			...
			lpMeasureItemStruct->itemWidth = cx + 20;   // we are done
		}
	}
	...
	void DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
	{
		...
		if (bSelected && !bDisabled)
		{
			...
			//pDC->Rectangle (pRect);
			ImageInfo* pImg = GetImage(IDB_MENUMARK);

			HDC hCloneDC = ::CreateCompatibleDC(pDC->m_hDC);
			HBITMAP hOldBitmap = (HBITMAP) ::SelectObject(hCloneDC, pImg->hBitmap);
			::SetStretchBltMode(pDC->m_hDC, COLORONCOLOR);
			BLENDFUNCTION bf = { AC_SRC_OVER, 0, 255, AC_SRC_ALPHA };

			AlphaBlend(pDC->m_hDC, pRect->left, pRect->top, pRect->right - pRect->left, pRect->bottom - pRect->top, hCloneDC,
				0, 0, 149, 21, bf);
		...
		else
		{
			// Draw the text
			WTL::CRect rc(pRect);
			CAtlString sCaption = pmd->lpstrText;
			...
			pDC->SetTextColor(bSelected ? m_ColorHotText : ::GetSysColor(COLOR_MENUTEXT));
			...
			if (bChecked)
			{
				...
				pDC->Rectangle(WTL::CRect(pRect->left + 1, pRect->top + 1, pRect->left + m_szButton.cx - 2, pRect->bottom - 1));
			}
			...
			if (m_ImageList != NULL && pmd->iImage >= 0)
			{
				...
				if (bDisabled || (bSelected && !bChecked))
				{
					...
					pDC->DrawState(WTL::CPoint(pRect->left + (bOver ? 4 : 3), rc.top + (bOver ? 5 : 4)),
						WTL::CSize(m_szBitmap.cx, m_szBitmap.cx), hIcon, DSS_MONO, brush);
					DestroyIcon(hIcon);
				}
				if (!bDisabled)
				{
			...
			else if (m_PngImageList != NULL && pmd->iImage >= 0)
			{
				HDC hCloneDC = ::CreateCompatibleDC(pDC->m_hDC);
				HBITMAP hOldBitmap = (HBITMAP) ::SelectObject(hCloneDC, m_PngImageList->hBitmap);
				::SetStretchBltMode(pDC->m_hDC, COLORONCOLOR);
				BLENDFUNCTION bf = { AC_SRC_OVER, 0, 255, AC_SRC_ALPHA };

				AlphaBlend(pDC->m_hDC, pRect->left + 3, pRect->top + (pRect->bottom - pRect->top - 16) / 2, 16, 16, hCloneDC,
					16 * (pmd->iImage - 1), 0, 16, 16, bf);
			...
			else if (bChecked)
			{
				...
				BOOL bRet = pDC->DrawState(WTL::CPoint(rc.left, rc.top + 3), WTL::CSize(rc.Size()), hBmp, DSS_NORMAL, (HBRUSH)NULL);
				DeleteObject(hBmp);
			}
		}
	}
...
public:
	...
	void GetSystemSettings()
	{
		...
		//	(LPTSTR) &lpMsgBuf,
		//	0, NULL );
		//wsprintf(szBuf,	_T("%s 出错信息 : %s"),_T("SystemParametersInfo"), lpMsgBuf);
		//LocalFree(lpMsgBuf);
		////输出提示。
		//if(!bRet)MessageBox(NULL,szBuf,_T("fuck"),0);
		...
		CWindowDC dc(static_cast<T*>(this)->GetHWND());
