...
bool CWindowWnd::RegisterWindowClass()
{
    ...
    wc.cbClsExtra = 0;
    wc.cbWndExtra = 0;
	wc.hIcon = ::LoadIcon(CPaintManagerUI::GetResourceInstance(),MAKEINTRESOURCE(IDI_FILMEVERYDAY));
...
BOOL CWindowWnd::MoveWindow(LPRECT rect,BOOL bRepaint)
{
	return ::MoveWindow(m_hWnd,rect->left,rect->top,rect->right - rect->left,rect->bottom - rect->top,bRepaint);
}

BOOL CWindowWnd::MoveWindow(int x,int y,int width,int height,BOOL bRepaint)
{
