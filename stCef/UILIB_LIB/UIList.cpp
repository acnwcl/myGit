...
CListUI::CListUI() : m_pCallback(NULL), m_iCurSel(-1), m_iExpandedItem(-1), m_iEnsure(-1), m_iShowIndex(-1)
{
...
void CListUI::Event(TEventUI& event)
{
	...
	{
		if(this->m_pList->IsFocused())
			m_pManager->SendNotify(this, MST_CONTEXTMENU);
	}
    switch( event.Type ) {
...
bool CListUI::SelectItems(int iIndex)
{
	CControlUI* pControl = GetItemAt(iIndex);
	if( pControl == NULL ) return false;
	if( !pControl->IsVisible() ) return false;
	...
	IListItemUI* pListItem = static_cast<IListItemUI*>(pControl->GetInterface(_T("ListItem")));
	if( GetKeyState( VK_SHIFT )&0x8000 )//shift按下
	{
		ClearSelects();
		if(m_iCurSel == -1) return SelectItem(iIndex);
		int i = iIndex>m_iCurSel?m_iCurSel:iIndex;
		int j = iIndex>m_iCurSel?iIndex:m_iCurSel;
		for(; i<=j; i++)
		{
			CControlUI* pControl = GetItemAt(i);
			if(pControl == NULL) return false;
			...
			pListItem->SelectMutli(true);
		}
		return true;
	}
	else if( GetKeyState( VK_CONTROL )&0x8000 )//ctrl按下
	{
		if(pListItem->IsSelected())
		{
			if(!pListItem->SelectMutli(false)) return false;
			if(m_iCurSel == iIndex)
				m_iCurSel = -1;
			pControl->SetFocus();
			return true;
		...
		{
			if(!pListItem->SelectMutli(true)) return false;
			m_iCurSel = iIndex;
			pControl->SetFocus();
			return true;
		}
...
void CListUI::DoPaint(HDC hDC, const RECT& rcPaint)
{
	...
	if(m_iEnsure!=-1)
		EnsureVisible(m_iEnsure,m_iShowIndex);
}

void CListUI::EnsureVisible(int iIndex,int iShowIndex)
{
	m_iEnsure = iIndex;
	m_iShowIndex = iShowIndex;
    if( m_iCurSel < 0 ) return;
	CControlUI *pCtrl = m_pList->GetItemAt(iIndex);
	if(pCtrl == NULL) return;
    RECT rcItem = pCtrl->GetPos();
	if(!::IsRectEmpty(&rcItem))
	{
		m_iEnsure = m_iShowIndex = -1;
	}
	...
	if(iShowIndex!=-1)
	{
		int top = rcList.top+(rcItem.bottom-rcItem.top)*iShowIndex;//iShowIndex所在的位置 top
		int bottom = top + rcItem.bottom - rcItem.top;
		if( rcItem.top < top ) dx = rcItem.top - top;
		if( rcItem.bottom > bottom ) dx = rcItem.bottom - bottom;
	}
	else
	{
...
void CListHeaderItemUI::Event(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK )
    {
        ...
        if( ::PtInRect(&rcSeparator, event.ptMouse) ) {
            ...
            m_pManager->SendNotify(this, MST_HEADERCLICK);
            Invalidate();
        }
        return;
    }
...
bool CListElementUI::SelectMutli(bool bSelect)
{
...
void CListLabelElementUI::Event(TEventUI& event)
{
	...
	{
        if( IsEnabled() ) {
			m_pManager->SendNotify(this, MST_ITEMCLICK);
			if(pInfo && pInfo->bMultiSelect)
			{
				if( m_pOwner != NULL ) ((IListUI*)m_pOwner)->SelectItems(m_iIndex);
				return ;
			}
            Select();
            Invalidate();
	...
	if( event.Type == UIEVENT_RBUTTONDOWN || event.Type == UIEVENT_RDBLCLICK )
	{
		if( IsEnabled() ) {
			m_pManager->SendNotify(this, MST_ITEMRCLICK);
			if(pInfo && pInfo->bMultiSelect && !m_bSelected)
			{
				if( m_pOwner != NULL ) ((IListUI*)m_pOwner)->SelectItems(m_iIndex);
			}
			else
...
bool CListExpandElementUI::Expand(bool bExpand)
{
    ...
    if( bExpand ) 
    {
        ...
        if( m_pManager != NULL ) m_pManager->SendNotify(this, MST_ITEMEXPAND);
        if( m_pSubControl ) m_pManager->InitControls(m_pSubControl, this);
    }
    else
    {
        if( m_pManager != NULL ) m_pManager->SendNotify(this, MST_ITEMCOLLAPSE);
    }
    return true;
}
...
bool CListContainerElementUI::SelectMutli(bool bSelect)
{
    if( !IsEnabled() ) return false;
    if( bSelect == m_bSelected ) return true;
    m_bSelected = bSelect;
